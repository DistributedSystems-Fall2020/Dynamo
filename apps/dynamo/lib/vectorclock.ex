defmodule Dynamo.VectorClock do
  ...
  def less_than(vector_clock_1, vector_clock_2) do
    ...
    if have_same_keys(clock_1, clock_2) do
      campare_result =
        Map.values(
          Map.merge(clock_1, clock_2, fn _k, c1, c2 ->
            compare_component(c1, c2)
          end)
        )
  ...
  @spec less_than_equal_to(%Dynamo.VectorClock{}, %Dynamo.VectorClock{}) ::
          boolean
  def less_than_equal_to(vector_clock_1, vector_clock_2) do
    less_than(vector_clock_1, vector_clock_2) or
      equal_to(vector_clock_1, vector_clock_2)
  end
  ...
  @spec greater_than_equal_to(%Dynamo.VectorClock{}, %Dynamo.VectorClock{}) ::
          boolean
  def greater_than_equal_to(vector_clock_1, vector_clock_2) do
    greater_than(vector_clock_1, vector_clock_2) or
      equal_to(vector_clock_1, vector_clock_2)
  end

  @spec merge_with_res(non_neg_integer(), %Dynamo.VectorClock{}, list()) ::
          any()
  ...
  defp coalesce_helper(curr_index, vector_clock_list, result) do
    if curr_index == length(vector_clock_list) do
      ...
      {result, succ} =
        merge_with_res(0, Enum.at(vector_clock_list, curr_index), result)

      result =
        if succ do
          result
        else
          result ++ [Enum.at(vector_clock_list, curr_index)]
        end
  ...
  @spec combine_vector_clocks(%Dynamo.VectorClock{}, %Dynamo.VectorClock{}) ::
          %Dynamo.VectorClock{}
  def combine_vector_clocks(vector_clock_1, vector_clock_2) do
    ...
    clock =
      Map.merge(clock_1, clock_2, fn _k, c, r -> combine_component(c, r) end)
  ...
  defp converge_helper(curr_index, vector_clock_list, result) do
    if curr_index == length(vector_clock_list) do
      ...
      result =
        combine_vector_clocks(Enum.at(vector_clock_list, curr_index), result)
  ...
  defp coalesce2_helper(curr_index, vector_clock_value_list, result) do
    if curr_index == length(vector_clock_value_list) do
      ...
      {result, succ} =
        merge_with_res2(0, Enum.at(vector_clock_value_list, curr_index), result)

      result =
        if succ do
          result
        else
          result ++ [Enum.at(vector_clock_value_list, curr_index)]
        end
